...
class Game extends Observable {
    ...
    private void setAuto(Side color) {
        if (p1.getSide() == color) {
            p1 = new AI(this, color);
        }
        if (p2.getSide() == color) {
            p2 = new AI(this, color);
        }
    }
    ...
    private void setManual(Side color) {
        if (p1.getSide() == color) {
            p1 = new HumanPlayer(this, color);
        }
        if (p2.getSide() == color) {
            p2 = new HumanPlayer(this, color);
        }
    }

    /** Return the Player playing COLOR. */
    private Player getPlayer(Side color) {
        Player p = new HumanPlayer(this, WHITE);
        if (p1.getSide() == color) {
            p = p1;
        }
        if (p2.getSide() == color) {
    ...
    private void setSpots(int r, int c, int spots, String color) {
        if (spots > _board.neighbors(r, c)) {
            System.out.println("Error: more spots than neighbors");
        } else {
            if (spots == 0) {
   ...
   /** Used to return a move entered from the console.  Allocated
    ...
    /** instance of player 1 P1. */
    private Player p1;
    /** instance of player 2 P2. */
    private Player p2;
}
