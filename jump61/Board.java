...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        if ((n < size() * size() && n >= 0) && ((get(n).getSide() == player)
            || (get(n).getSide() == WHITE))) {
            return true;
        }
        return false;
    ...
    final Side getWinner() {
        Side winning = get(0).getSide();
        for (int i = 0; i < size() * size(); i++) {
            if (get(i).getSide() != winning) {
                return null;
            }
        }
        if (winning == WHITE) {
    ...
    public String toString() {
        ...
        for (int n = 0; n < size() * size(); n++) {
            ...
            if (get(n).getSide() == WHITE) {
                System.out.print(get(n).getSpots() + "-" + " ");
            }
            if (get(n).getSide() == RED) {
                System.out.print(get(n).getSpots() + "r" + " ");
            }
            if (get(n).getSide() == BLUE) {
                System.out.print(get(n).getSpots() + "b" + " ");
            }
            if ((n + 1) % size() == 0) {
                System.out.println("");
            }
